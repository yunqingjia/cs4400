...
sp_main2: BEGIN
    ...
    -- check to see if password is more than 8
    ...
    If(i_balance is NULL and i_email is NULL)
    THEN DELETE FROM USER WHERE username = i_username;
    LEAVE sp_main2;
    end if;
    
    
    -- check to see if customer and balance is over 0
	IF(i_balance is NOT NULL)
    THEN
	IF(i_balance <= 0)
	THEN 
    ...
    -- creating an employee if email is added
    IF(i_email is NOT NULL)
    THEN
    INSERT INTO employee VALUES (i_username, i_email);
    IF(i_type = 'Admin') 
    THEN INSERT INTO Admin VALUES (i_username);
    END IF;
	IF(i_type = 'Manager') 
    THEN INSERT INTO Manager VALUES (i_username);
    END IF;
    IF(i_type = 'Staff') 
    THEN INSERT INTO Staff VALUES (i_username);
    END IF;
	END IF;
...
DROP VIEW IF EXISTS station_num_foodTruck_view;
CREATE VIEW station_num_foodTruck_view AS SELECT FOODTRUCK.stationName, COUNT(FOODTRUCK.stationName) AS num_foodTruck FROM FOODTRUCK GROUP BY FOODTRUCK.stationName;
...
BEGIN
	...
	CREATE TABLE mn_filter_foodTruck_result(foodTruckName varchar(100), stationName varchar(100), remainingCapacity INT, staffCount int, menuItemCount int)
	SELECT (capacity - num_foodTruck) as remainingCapacity, FoodTruck.foodTruckName, STATION.stationName, capacity, COUNT(DISTINCT username) as staffCount, COUNT(DISTINCT foodName) as menuItemCount
    FROM FoodTruck
    INNER JOIN STATION
    ...
    INNER JOIN station_num_foodTruck_view
    ON FoodTruck.stationName = station_num_foodTruck_view.stationName
    WHERE
    (i_managerUsername = managerUsername) AND
    (i_foodTruckName = FoodTruck.foodTruckName OR i_foodTruckName is NULL OR FoodTruck.foodTruckName LIKE CONCAT("%", i_foodTruckName, "%")) AND
    (i_stationName = Station.stationName OR i_stationName is NULL OR Station.stationName LIKE CONCAT("%", i_stationName, "%")) AND
    ((i_hasRemainingCapacity = TRUE AND (capacity-num_foodTruck)>0) OR (i_hasRemainingCapacity = FALSE))
    GROUP BY foodTruckName
    HAVING 
    ((i_minStaffCount IS NULL AND i_maxStaffCount IS NULL) 
    OR (i_minStaffCount IS NULL AND staffCount <= i_maxStaffCount) 
    OR (i_maxStaffCount IS NULL AND i_minStaffCount <= staffCount) 
    OR (staffCount BETWEEN i_minStaffCount AND i_maxStaffCount));
...
IF (SELECT STAFF.foodTruckName IS NULL FROM STAFF WHERE STAFF.username = i_staffName) THEN
	UPDATE STAFF
...
SELECT station.stationName, building.buildingName, tags, `description`, balance
	FROM customer inner join station on customer.stationName = station.stationName
    inner join building on station.buildingName = building.buildingName
    inner join (SELECT B.buildingName, GROUP_CONCAT(BT.tag) as "tags" FROM Building as B LEFT JOIN BuildingTag as BT on B.buildingName = BT.buildingName GROUP BY B.buildingName) as A on
    A.buildingName = station.buildingName
...
BEGIN
    ...
    select foodTruck.foodTruckName, manager, foods from customer inner join station on customer.stationName = station.stationName
    inner join foodtruck on foodtruck.stationName = station.stationName
    inner join (select menuitem.foodTruckName, GROUP_CONCAT(menuitem.foodName SEPARATOR ',') as 'foods' from menuitem left join foodTruck on menuitem.foodtruckname = foodtruck.foodtruckname
    group by foodTruck.foodTruckName) as A on A.foodTruckName = foodTruck.foodTruckName
    inner join (select manager.username, GROUP_CONCAT(User.firstName, ' ', User.lastName) as 'manager' from user join manager on user.username = manager.username group by manager.username)
    as B on B.username = foodTruck.managerUsername
    where i_customerUsername = customer.username;
...
BEGIN
    ...
    SET @foodPrice = (select price from menuitem where (i_foodTruckName = menuitem.foodTruckName) AND
 	(i_foodName = menuitem.foodName));
    SET @priceTotal = @foodPrice*i_purchaseQuantity;
    SET @custUsername = (select customerUsername from orders where i_orderId = orders.orderID);
    SET @balance = (select balance from customer where (@custUsername = username));
    ...
    update customer SET balance = @balance - @priceTotal where username = @custUsername;
    END IF;
...
BEGIN
	DROP VIEW IF EXISTS cus_OH ;
	CREATE VIEW cus_OH as 
	Select OrderID, sum(purchaseQuantity*price) as 'OrderTotal', group_concat(DISTINCT orderdetail.foodName ORDER BY orderdetail.foodName ASC SEPARATOR ',') as 'foodNames', sum(orderdetail.purchaseQuantity) as 'foodQuantity'
	from orderdetail 
	...
	Select date, orderID, orderTotal, foodNames, foodQuantity
	from orders natural join cus_OH
	where i_customerUsername = customerUsername; 
